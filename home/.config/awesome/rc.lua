...
myawesomemenu = {
   ...
   { "poweroff system", "shutdown -p now" }
}
...
globalkeys = awful.util.table.join(
	awful.key({modkey}, "F1", function() awful.client.jumpto(awful.client.getmaster()) end),
...
-- My Keybindungs
    ...
    awful.key({ modkey,                 }, "i", function () awful.util.spawn_with_shell("uxterm -title irc -e /usr/bin/ssh -t -p1980 server ~/bin/start_tmux irc /usr/local/bin/weechat") end),
    awful.key({ modkey,                 }, "c", function () awful.util.spawn_with_shell("uxterm -title server -e /usr/bin/ssh -t -p1980 server ~/bin/start_tmux") end),
    awful.key({ modkey, "Control"   	}, "0", function () awful.util.spawn_with_shell("xlock -echokeys -echokey \"~\" -mode bat -mousemotion") end),
...
clientkeys = awful.util.table.join(
	awful.key({ modkey },            "F3",     function (c) 
							awful.prompt.run({ prompt = "Rename client: "},
									mypromptbox[mouse.screen].widget,
							function (s)
							c:set_xproperty("WM_NAME", s) 
							end)
						  end),
    ...
    awful.key({ modkey, "Control" }, "Return", function (c) 
								local clientToSwap = nil
								if (awful.client.getmaster() == c) then
									clientToSwap=lastMaster[awful.tag.selected(mouse.screen)] or awful.client.getmaster()
								else
									clientToSwap = c
								end
								lastMaster[awful.tag.selected(mouse.screen)] = awful.client.getmaster()
								clientToSwap:swap(awful.client.getmaster())
								awful.client.jumpto(clientToSwap)
							end),
